...
/*         PromptManager.ExecutePromptPipeline(new OpenAI.Gpt4Turbo(), includeSrcCode);
    ...
    public static void NewMain()
    {
        Prompt prompt = new()
        {
            AiModel = ChatGPT.Gpt4o,
        };
        MessageBlock messageBlock = new()
        {
            Input = PromptManager.LoadPrompt()
        };
        messageBlock.GenerateContent();
        prompt.MessageBlocks.Add(messageBlock);
        Completion completion = prompt.CompletePrompt();
        PromptManager.WriteCompletion(completion.Response);
    }
...
public class ProgramPaths
{
    ...
    public static readonly string Completion = Root;
...
public class ProgramFiles
{
    ...
    public static readonly string Completion = "response.md";
...
{
    public static readonly string AbsolutePath = @"C:\Users\BudoB\OneDrive\Dokumenter Tekst\Programmering\Spiludvikling\Godot\Prompt\";
